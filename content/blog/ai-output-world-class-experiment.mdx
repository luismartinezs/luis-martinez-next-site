...
metadescription: "A three-month experiment in context engineering, multi-agent pipelines, and the gap between good enough and genuinely excellent AI output. 500+ LLM calls, $15 in API fees, and five findings that changed how I build."
date: "2026-04-09"
...
*A three-month experiment in context engineering, multi-agent pipelines, and the gap between "good enough" and genuinely excellent AI output.*
...
Better. Obviously better. But I hadn't changed the model or the context. I had just given it better direction. Which raised the question that consumed the next several months:
...
I started with code debugging, then landing page copy, then UI design. The experiment ran for several months, 500+ LLM calls, and cost roughly $15 in API fees.
...
Everything described here is open source and in a single repository. The ablation runner, JSONL trace registry, multimodal judge, routing pipeline, and Playwright screenshot infrastructure are all domain-agnostic. If you want to test whether a KB helps or hurts your specific pipeline, or measure the contribution of a multi-agent layer, the tooling is there.

The total cost to run all experiments described in this post: $15 in API fees and 8 hours of compute time across several months of iteration.
